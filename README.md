...
Docker container with Apache PHP 7.2 build by [Nix package manager](https://nixos.org/).
...
## Building Apache PHP 7.2

To build Apache PHP 7.2, follow these steps:
...
[![Build Status](https://jenkins.majordomo.ru/job/webservices/job/apache2-php72/job/master/lastBuild/badge/icon?config=webservices-apache2-php72)](https://jenkins.majordomo.ru/job/webservices/job/apache2-php72/job/master/lastBuild/badge/icon?config=webservices-apache2-php72)

## Using Apache PHP 7.2

To use Apache PHP 7.2, call:

``` shell
docker inspect docker-registry.intr/webservices/apache2-php72 | grep cmd
```
...
docker run --read-only --network=host \
        ...
        --ulimit stack=-1:-1 \
        --cap-add SYS_ADMIN \
        --security-opt apparmor:unconfined docker-registry.intr/webservices/apache2-php72:latest
```
...
* `$SOCKET_HTTP_PORT` any free port on your computer, e.g. `8072`.

## Contributing to Apache PHP 7.2

To contribute to Apache PHP 7.2, follow these steps:
